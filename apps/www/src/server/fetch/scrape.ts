...
import { FetchScrapperService } from "./scrappers/axios";
import { excludeNonMainTags } from "@/global/excludeTags";
  ...
  async FetchURL({
    ...
    switch (scrapper) {
      case "axios":
        ...
        this.cleanupHtml($ as cheerio.CheerioAPI);
  ...
  async scrapeAllSites(
    ...
    try {
      ...
      if (error instanceof Error) {
        ...
        throw new Error(
          `Failed to scrape sites from ${domain}: ${String(error)}`,
        );
      }
    }
  }
